...
public class PostAddSaleViewController {
    ...
    @FXML
    private TextField minimumRaise;
    ...
    void handleAddEvent(ActionEvent event) {
    	...
    	double t_askingPrice, t_minimumRaise;
    	try {
    		t_description = description.getText();
    		t_title = title.getText();
    		t_askingPrice = Double.parseDouble(askingPrice.getText());
    		t_minimumRaise = Double.parseDouble(minimumRaise.getText());
    	...
    	Sale sale = new Sale(Sale.getEventUniqueId(), t_title, t_description, UniLinkSession.username, t_askingPrice, t_minimumRaise);
    	sale.setImage(image);
    	try {
			DBTool.addSale(sale);
